...
export function ReportView({ id }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                const response = await fetch(`${CONFIG.apiUrl}/Report/report?id=${id}&reportType=2`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <DashboardContent maxWidth="xl">
            <CustomBreadcrumbs
                ...
                backHref={paths.dashboard.report.analytics.root}
                links={[
                    { name: 'Başlagıç', href: paths.dashboard.root },
                    { name: 'Rapor' },
                    { name: 'Analytics', href: paths.dashboard.report.analytics.root },
