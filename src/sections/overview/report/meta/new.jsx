...
export function MetaReportNew() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const token = localStorage.getItem('jwtToken');
      const response = await fetch(`${CONFIG.apiUrl}/Report/chatgpt-prompt-meta?name=${name}&account=${accounts.find((acc) => acc.accountId === selectedAccount)?.account}&accountId=${selectedAccount}&typeId=${selectedFilter}&reportType=1&startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
            window.location.href = '/dashboard/report/meta';
        }, 1000);
      } else {
  ...
  return (
    <DashboardContent>
        ...
        links={[
            ...
            { name: 'Rapor' },
            { name: 'Meta Raporları', href: paths.dashboard.report.meta.root },
            { name: 'Rapor Oluştur' },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
        />
                        ...
                        <Grid size={{ xs: 12, lg: 12 }}>
                            <FormControl fullWidth>
                                ...
                                >
                                    ...
                                    <MenuItem value="Reklam Seti">Reklam Seti</MenuItem>
                                    <MenuItem value="Reklam">Reklam</MenuItem>
                                </Select>
                            </FormControl>
                        </Grid>
