...
export function ReportView({ id }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                const response = await fetch(`${CONFIG.apiUrl}/Report/report?id=${id}&reportType=1`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <DashboardContent maxWidth="xl">
            <CustomBreadcrumbs
                ...
                backHref={paths.dashboard.report.meta.root}
                links={[
                    { name: 'Başlagıç', href: paths.dashboard.root },
                    { name: 'Rapor' },
                    { name: 'Meta', href: paths.dashboard.report.meta.root },
