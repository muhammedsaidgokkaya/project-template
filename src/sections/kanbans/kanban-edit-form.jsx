...
export function KanbanEditForm({ currentTour }) {
  const [departments, setDepartments] = useState([]);
  const [selectedDepartments, setSelectedDepartments] = useState(currentTour?.departments);
  const [users, setUsers] = useState([]);
  const [selectedUsers, setSelectedUsers] = useState(currentTour?.users);
  const [loading, setLoading] = useState(false);
  const [tourServiceOptions, setTourServiceOptions] = useState([]);
  const [selectedServiceOptions, setSelectedServiceOptions] = useState(currentTour?.services);
  ...
  useEffect(() => {
    if (currentTour) {
      ...
      setTimeout(() => {
        setSelectedDepartments(currentTour.departments || []);
        setSelectedUsers(currentTour.users || []);
        setSelectedServiceOptions(currentTour.services || []);
      }, 0);
    }
  }, [currentTour, reset]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {  
      ...
      const response = await fetch(`${CONFIG.apiUrl}/Task/update-task`, {
        method: "POST",
        headers: {
